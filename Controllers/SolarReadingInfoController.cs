...
public class SolarReadingInfoController : Controller
{
    private readonly ApplicationDbContext _context;
    private readonly UserManager<ApplicationUser> _userManager;

    public SolarReadingInfoController(ApplicationDbContext context, UserManager<ApplicationUser> userManager)
    {
        _context = context;
    ...
    [HttpGet]
    public async Task<IActionResult> SolarReadingInfoList(int page = 1, string searchString = "")
    {
        int pageSize = 15;
    ...
    {
        //var model = new TblSolarReadingInfo
        //{
        ...
        var lastTrDate = _context.TblSolarReadingInfos
            .Include(x => x.Eq)
    ...
    public async Task<IActionResult> Create(TblSolarReadingInfo solarReadingInfo)
    {
        try
        {
            ...
            _context.TblSolarReadingInfos.Add(solarReadingInfo);
            await _context.SaveChangesAsync();

            TempData["SuccessMessage"] = "Reading created successfully.";

            return RedirectToAction(nameof(SolarReadingInfoList));
        }
        catch (Exception)
        {
    ...
    {
        var reading = await _context.TblSolarReadingInfos
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(TblSolarReadingInfo solarReadingInfo)
    {
        try
        {
            if (!ModelState.IsValid)
    ...
    {
        var data = await _context.TblSolarReadingInfos.FindAsync(id);
    ...
    {
        var data = await _context.TblSolarReadingInfos
            .FirstOrDefaultAsync(x => x.Trid == id);
        ...
        _context.TblSolarReadingInfos.Remove(data);
        await _context.SaveChangesAsync();
